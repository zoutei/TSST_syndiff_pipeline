> **Package integration**: `syndiff` stage **`downsample`** · modules `template_creation/processing/field_downsample.py` (field) and `linear_downsample.py` (linear) · product path `templates/oversampling_{N}/` · legacy script `multi_offset_downsampling.py`. Scheduler stage names `templates` / `tmpl` are rejected; the `templates/` name is a product directory only.
...
In the supervised pipeline, this algorithm runs as the `downsample` stage, an SCC-scoped template stage. For `geometry_mode: field` (default), templates and diff geometry come from the SCC `MappingGrid` via `field_mode_assembly.json` and `scc_bootstrap` — no per-event handoff. L5 now fails before writing contribs if any required master skycell is absent from the convolved store; it does not skip that batch. For `geometry_mode: linear`, per-transient offsets are resolved from event-scoped config at template build time. Template output is written to `{data_root}/s{SSSS}/c{C}/k{K}/templates/oversampling_{N}/` (`scc_templates_dir()`). See [field_geometry.md](../field_geometry.md), [coordinate frames and cropping](../coordinate_frames_and_cropping.md), and [template_pipeline.md](../template_pipeline.md).
