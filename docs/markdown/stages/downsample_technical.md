...
In the supervised pipeline, this algorithm runs as the `templates` stage (config key/legacy alias: `downsample`), an SCC-scoped template stage — not the event-scoped `bind` stage (the current name for what this document calls `wcs_grouping`). For `geometry_mode: linear`, `templates` reads crop bounds and per-transient offsets from `bind`'s `event_job.json` (legacy name: `cluster_template_job.json`) if that event's handoff already exists on disk. Template output is written to the SCC's shared store under `{data_root}/scc/s{SSSS}_c{C}_k{K}/templates/oversampling_{N}/` (`common/scc_paths.py::scc_templates_dir()`) — there is **no** `ws/templates` symlink; `diff` resolves the template directory from that SCC path directly. `templates` also writes `ps1_removed_stars.csv` in the event dir. See the [template pipeline guide](../template_pipeline.md) for handoff inputs, verify rules, and the path to `diff`.
