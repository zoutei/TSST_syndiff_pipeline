...
In the supervised pipeline, this algorithm runs as the `downsample` stage (config alias: `templates`), an SCC-scoped template stage. For `geometry_mode: field` (default), templates and diff geometry come from the SCC `MappingGrid` via `field_mode_assembly.json` and `scc_bootstrap` — no per-event handoff. For `geometry_mode: linear`, per-transient offsets are resolved from event-scoped config at template build time. Template output is written to `{data_root}/s{SSSS}/c{C}/k{K}/templates/oversampling_{N}/` (`scc_templates_dir()`). See [field_geometry.md](../field_geometry.md) and [template_pipeline.md](../template_pipeline.md).
