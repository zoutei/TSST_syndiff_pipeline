...
8. [Download then fpack to `.fits.fz`](#8-download-then-fpack-to-fitsfz)
...
| Provenance | `SPOC` |
| Typical file size | Tens of MB on disk as `.fits.fz` (MAST usually serves FITS+GZIP; we gunzip then fpack) |
| Image shape | 2136 × 2078 pixels (science array in HDU 1) |
| Cadence | Depends on TESS cycle (30 min, 10 min, or 200 s) |

**Filename pattern** (manifest basename, before local fpack suffix):
...
On disk the pipeline stores **`*.fits.fz`** (CFITSIO fpack). Legacy `*.fits.gz` and plain `*.fits` remain readable.
...
flowchart LR
    ...
    DL --> Cache["{data_root}/tess_ffi/sNNNN/camC_ccdK/*.fits.fz"]
...
| [`syndiff_pipeline/common/fits_io.py`](../../syndiff_pipeline/common/fits_io.py) | `fpack` write helpers used after download |
| [`syndiff_pipeline/common/fits_variants.py`](../../syndiff_pipeline/common/fits_variants.py) | Tri-format path resolution (`.fz` / `.gz` / plain) |
...
| [`tests/test_download_ffis.py`](../../tests/test_download_ffis.py) | Unit tests for fpack paths, streaming, parallelism |
...
/astro/armin/koji/syndiff/data/tess_ffi/s0020/cam3_ccd3/
  tess2020019142923-s0020-3-3-0165-s_ffic.fits.fz
  tess2020019142924-s0020-3-3-0166-s_ffic.fits.fz
  ...
...
The **tesscurl manifest** lists basenames ending in `_ffic.fits`. Locally we store `_ffic.fits.fz`.

| Context | Example basename |
|---------|------------------|
| tesscurl manifest / MAST product table | `tess..._ffic.fits` |
| On disk (current) | `tess..._ffic.fits.fz` |
| Legacy on disk (still supported) | `tess..._ffic.fits.gz`, `tess..._ffic.fits` |

Mapping helpers:

- `spoc_ffi_fpack_basename("…_ffic.fits")` → `…_ffic.fits.fz`
- `manifest_basename_from_local("…_ffic.fits.fz")` → `…_ffic.fits`

Skip logic and verification compare **manifest basenames**, so a local `.fits.fz` (or legacy `.fits.gz` / `.fits`) satisfies a manifest entry for `.fits`.

### Glob and discovery

`list_local_ffis(ffi_dir, sector, camera, ccd)`:

1. Globs `*_ffic.fits.fz` first, then `*_ffic.fits.gz`, then plain `*_ffic.fits`.
2. If multiple variants exist for the same product, **prefers `.fits.fz`**.
...
flowchart TD
    ...
    Parallel --> Stream[_stream_url_to_fpack_fits per file]
    ...
    Parallel2 --> AQ[Observations.download_file + fpack]
    AQ --> List
```
...
This avoids a `curl` subprocess dependency and gives full control over streaming, compression, and parallelism.
...
| 6 | Build tasks | One closure per file calling `_stream_url_to_fpack_fits` |
...
Each download stages bytes through a temp plain `.fits` (or `.part`), then `fpack` writes the final `*.fits.fz`. On failure temps are removed. This prevents downstream stages from reading half-written FITS.
...
| 7 | `compress_spoc_ffi_to_fpack` — fpack to `.fits.fz`, delete plain `.fits` |
...
| Compression | Stream → plain → fpack | Plain `.fits` then fpack |
| Dependency | stdlib + `fpack` on PATH | `astroquery` + `fpack` |
| Parallelism | Yes (`max_workers`) | Yes (`max_workers`) |

---

## 8. Download then fpack to `.fits.fz`

Function: `_stream_url_to_fpack_fits(url, fz_dest_path, timeout)`.

### Why not stream MAST gzip bytes into `.fits.fz`?

`.fits.fz` is CFITSIO **tile-compressed** FITS (via `fpack`), not a gzip wrapper. MAST often already serves gzip-compressed FITS payloads. Those bytes must be gunzipped to plain FITS before `fpack` can produce a valid `.fits.fz`.
...
flowchart TD
    ...
    Magic -->|Yes MAST FITS+GZIP| Gunzip[Gunzip to temp plain .fits]
    Magic -->|No uncompressed FITS| Plain[Write chunks to temp plain .fits]
    Gunzip --> Fpack["fpack -g -q 0 -Y -D"]
    Plain --> Fpack
    Fpack --> Done["*.fits.fz on disk; plain removed"]
```

**MAST typically serves pre-gzip-compressed bytes** even though tesscurl basenames end in `.fits`. The magic-byte peek (`\x1f\x8b`) detects this and gunzips before fpack.

`fpack` must be on `PATH` (provided by the `syndiff` conda env / CFITSIO). Writers use lossless GZIP tile compression (`-g -q 0`).

### Migration helpers

- `compress_spoc_ffi_to_fpack` — convert a plain `.fits` to `.fits.fz`.
- [`scripts/fpack_fits_migration.py`](../../scripts/fpack_fits_migration.py) — convert existing `.fits.gz` trees (`--scope tess_ffi|data_root|workspace`); deletes `.gz` after a verified write.
...
Before downloading, the code builds `existing = local_ffi_manifest_basenames(list_local_ffis(...))` and **drops any manifest basename already present** locally (whether stored as `.fits.fz`, `.fits.gz`, or `.fits`).
...
Files only appear in the final glob after a successful fpack write. Temps left by a killed worker are **not** counted as complete; the next run re-downloads that file.
...
Downstream code should open FFIs through the shared helpers (resolves `.fits.fz` / `.fits.gz` / `.fits` and sets `memmap=False` for compressed files):
...
# → …/tess2020019142923-s0020-3-3-0165-s_ffic.fits.fz if present
#    (else .fits.gz / .fits)
```

Pipeline per-epoch science outputs use a **different naming scheme** (`ffi_naming.py`): `{ffi_product_id}_{workspace_label}.fits.fz` under `events/{label}/ws/`. The `tess_ffi/` cache retains original SPOC basenames.

---

## 15. Performance notes (NFS and workers)

Benchmarks on this project's NFS (`/astro/armin/koji/syndiff/…`) with 8 FFIs from sector 22 (median of 2 repeats; historically measured for the gzip stream path):
...
Fpack adds a CPU step after the network download; wall time is still dominated by NFS/MAST for typical worker counts.
...
### `fpack not found on PATH`

- Activate the `syndiff` conda env (CFITSIO provides `fpack` / `funpack`).

### Corrupt / unreadable `.fits.fz`

- Re-download with `--overwrite` for affected basenames.
- Or migrate from a known-good `.fits.gz` with `scripts/fpack_fits_migration.py`.
...
### Legacy plain `.fits` or `.fits.gz` in cache

- Still discovered by `list_local_ffis` (`.fits.fz` preferred when both exist).
- Optional migration:

```bash
python scripts/fpack_fits_migration.py --scope tess_ffi --dry-run --limit 5
python scripts/fpack_fits_migration.py --scope tess_ffi
```
...
| Function | Description |
|----------|-------------|
| `spoc_ffi_fpack_basename(name)` | `…_ffic.fits` → `…_ffic.fits.fz` |
| `manifest_basename_from_local(path)` | `…_ffic.fits.fz` / `.gz` → `…_ffic.fits` |
| `resolve_local_ffi_path(directory, spoc_basename)` | Prefer `.fits.fz`, then `.gz`, then plain |
...
| Function | Description |
|----------|-------------|
| `_stream_url_to_fpack_fits(url, fz_dest, timeout)` | Single-file download → fpack |
| `_execute_ffi_downloads(tasks, max_workers)` | Parallel/sequential task runner |
| `compress_spoc_ffi_to_fpack(plain_path)` | Migrate plain `.fits` → `.fits.fz` |
...
- Migration: [`scripts/fpack_fits_migration.py`](../../scripts/fpack_fits_migration.py)
