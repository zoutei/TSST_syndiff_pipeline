...
**Partial runs** (`--stages mapping,ps1_process,templates`):
...
| `syndiff progress` | SQLite counts + `stage_progress.py` log parsing + `downsample.progress.json` (sidecar filename unchanged by the `templates` stage rename) |
...
| `syndiff_pipeline/cli.py` | Noun/verb entry (`template|diff submit|run`, delegates `star` and other verbs; `all` prints a removal error) |
