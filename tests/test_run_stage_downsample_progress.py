...
from syndiff_pipeline.template_creation.processing.downsample_progress import PROGRESS_FILENAME as DOWNSAMPLE_PROGRESS_FILENAME
from syndiff_pipeline.template_creation.processing.remap_progress import PROGRESS_FILENAME as REMAP_PROGRESS_FILENAME
...
class TestRunStageDownsampleProgressPath(unittest.TestCase):
    def test_execute_stage_receives_progress_path_for_downsample(self):
        ...
        expected = f"/runs/run_a/per_target/t1/{DOWNSAMPLE_PROGRESS_FILENAME}"
        self.assertEqual(captured["progress_path"], expected)

    def test_execute_stage_receives_progress_path_for_remap(self):
        captured: dict = {}
        ...
        self.assertEqual(rc, 0)
        self.assertEqual(captured["stage"], "remap")
        expected = f"/runs/run_a/per_target/t1/{REMAP_PROGRESS_FILENAME}"
        self.assertEqual(captured["progress_path"], expected)
