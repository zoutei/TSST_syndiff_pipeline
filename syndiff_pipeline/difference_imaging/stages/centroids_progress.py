"""Sidecar JSON progress for centroids frame processing.

Workspace copy lives under the centroids output dir
(``ws/centroids_r1/centroids.progress.json``). CLI mirror stays beside
``per_target/<label>/diff.log`` as ``diff.centroids.progress.json`` (used by
``syndiff progress`` / stage_progress).

Parent process updates sidecars after each frame completes. ``syndiff progress``
also merges live ``*_photresults.ecsv`` counts when *output_dir* is recorded.
"""
...
from syndiff_pipeline.difference_imaging.stages.centroids import PHOTRESULTS_ECSV_SUFFIX

PROGRESS_FILENAME = "centroids.progress.json"
CLI_PROGRESS_FILENAME = "diff.centroids.progress.json"
ARTIFACT_MERGE_STALE_SECONDS = 30.0


def progress_path_for_output_workspace(output_dir: Path | str) -> Path:
    """Canonical per-pass sidecar under ``ws/{centroids_label}/``."""
...
def count_photresults_artifacts(output_dir: Path | str) -> int:
    """Count per-frame ``*_photresults.ecsv`` files on disk."""
    root = Path(output_dir).expanduser().resolve()
    if not root.is_dir():
        return 0
    return sum(1 for _ in root.glob(f"*{PHOTRESULTS_ECSV_SUFFIX}"))
...
def init_progress(
    ...
    *,
    ...
    """Create or reset a centroids progress sidecar before frame processing."""
    payload = {
...
def merge_progress_with_artifacts(
    ...
    *,
    ...
    """
    Raise ``frames_done`` / ``frames_ok`` to match on-disk ECSV count.
    ...
    if not force and not _sidecar_is_stale(data):
        return data
    artifact_n = count_photresults_artifacts(out_dir)
    if artifact_n <= 0:
        return data
    merged = dict(data)
...
def refresh_progress_pair_from_artifacts(
    ...
    """Sync sidecar counters to the ECSV artifact count (parent, end of run)."""
...
def format_progress_text(data: dict[str, Any]) -> Optional[str]:
    ...
    label = str(data.get("centroids_label", "?"))
    ...
    if phase == "complete":
        ...
        return f"centroids {label} complete {ok}/{total}"
    return f"centroids {label} {done}/{total}"
