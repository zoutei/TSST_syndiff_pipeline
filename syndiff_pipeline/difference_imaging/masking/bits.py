...
# ePSF may use catalog stars (very bright, mid-bright, faint squares).
EPSF_IGNORE_BITS = BRIGHT_CAT | SAT_CROSS | FAINT_CAT  # 1 | 2 | 32
# Hotpants ignores Gaia faint squares (>~13); still masks bright tiers/straps/etc.
HOTPANTS_IGNORE_BITS = FAINT_CAT  # 32
STRAP_SOURCE_BITS = BRIGHT_CAT | SAT_CROSS | FAINT_CAT  # 1 | 2 | 32
...
def epsf_reject_mask(mask: np.ndarray) -> np.ndarray:
    ...
    Ignores all star stamps (bits 1|2|32: very bright / mid-bright / faint squares).
...
def hotpants_mask_bool(mask: np.ndarray) -> np.ndarray:
    ...
    Ignores bit 32 (``FAINT_CAT`` / Gaia mag ≳13 squares). Still masks very
    bright (bit 1), mid-bright (bit 2), straps, edges, PS1, TNS, asteroids, etc.
    """
    return (np.asarray(mask).astype(np.int64) & ~HOTPANTS_IGNORE_BITS) != 0


def strap_source_mask(mask: np.ndarray) -> np.ndarray:
    """Catalog sources for strap QE / phot_mask (bits 1|2|32)."""
