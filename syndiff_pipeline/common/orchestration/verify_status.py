...
def write_verify_in_flight(
    workspace_root: str | Path, by_run: dict[str, int | dict[str, Any]]
) -> None:
...
def read_verify_run_status(
    workspace_root: str | Path, run_id: str
) -> dict[str, Any]:
    """Return artifact-scan observability for one run."""
    by_run = _load_payload(workspace_root).get("by_run", {})
    return _normalize_run_entry(by_run.get(run_id, 0))


def read_verify_in_flight(
    workspace_root: str | Path, run_id: str | None = None
) -> int:
...
def read_verify_pending(workspace_root: str | Path, run_id: str) -> int:
    """Return queued artifact-scan count for *run_id*."""
    return read_verify_run_status(workspace_root, run_id)["scan_queued"]
...
def read_verify_active_keys(
    ...
    active = read_verify_run_status(workspace_root, run_id).get("active", [])
...
def refresh_verify_run_status(
    workspace_root: str | Path,
    state: object,
    run_id: str,
) -> None:
    """Refresh host-local verify JSON for *run_id* from live daemon worker state.

    No-op when the verify worker is not initialized (e.g. CLI-only context).
    """
    from syndiff_pipeline.common.orchestration.verify_worker import try_get_verify_worker

    if try_get_verify_worker() is None:
        return
    from syndiff_pipeline.common.orchestration.scheduler import collect_verify_status_for_run

    entry = collect_verify_status_for_run(state, run_id)
    if entry is None:
        return
    payload = _load_payload(workspace_root)
    by_run = dict(payload.get("by_run") or {})
    by_run[run_id] = entry
